...
if [ ! -d /home/pi/home_automation/.git ]; then
	if git clone git@github.com:IainBate/home-automation.git /home/pi/home_automation; then
		...
		echo ">>> slave_wifisn, feeds scripts/solax_cloud_data_logger.py) was only"
		echo ">>> added 2026-09-02 - if it predates that in the HDD backup you"
		echo ">>> restored from, it'll still be the NOT_USED_FOR_MODBUS"
		echo ">>> placeholder. Get a token from https://www.solaxcloud.com/#/api"
		echo ">>> and fill it in. NOTE: the token_id added on 2026-09-02 already"
		echo ">>> came back 'token invalid!' (API error code 103) the same day -"
		echo ">>> confirm the one you restore/enter actually works by running"
		echo ">>> 'venv/bin/python3 scripts/solax_cloud_data_logger.py --summary'"
...
echo "  [ ] verify secrets.yaml's solaX_cloud_api.token_id is present AND"
echo "      actually valid (run 'venv/bin/python3"
echo "      scripts/solax_cloud_data_logger.py --summary' and check for a"
echo "      'token invalid!' error) - feeds the nightly cron job in"

...
if [ ! -d /home/pi/home_automation/.git ]; then
	if git clone git@github.com:IainBate/home-automation.git /home/pi/home_automation; then
		...
		echo ">>> It installs FOUR services: the battery daemon, hot water"
		echo ">>> daemon, dashboard, and the Ohme status poller (which holds one"
		echo ">>> Ohme session and caches status for the other three - without"
		echo ">>> it they each log in to Ohme on every poll). It also installs"
		echo ">>> the repo's pre-push test gate via scripts/install_git_hooks.sh."
		...
		if [ ! -f /home/pi/home_automation/secrets.yaml ]; then
			if [ -f /home/pi/home_automation/secrets.yaml.enc ]; then
				...
				echo ">>> With no terminal to type into (an agent running this over"
				echo ">>> SSH, as this script's header expects), pass it instead:"
				echo ">>>   SECRETS_BACKUP_PASSPHRASE='...' bash scripts/decrypt_secrets.sh"
			else
		...
		echo ">>> added 2026-09-02 - if it predates that in the HDD backup you"
		echo ">>> restored from, it'll still be the NOT_USED_FOR_MODBUS"
		echo ">>> placeholder, OR the wrong KIND of token (see"
		echo ">>> src/api_clients/solax_cloud_client.py's module docstring - it"
		echo ">>> must be the plain numeric tokenID from the account's"
		echo ">>> 'Residential API (OLD)' page, NOT the 'API' page's OAuth2 App"
		echo ">>> Code, which looks superficially similar but always fails"
		echo ">>> 'token invalid!'). Confirm whichever value you restore/enter"
		echo ">>> actually works by running:"
		echo ">>>   venv/bin/python3 scripts/solax_realtime_logger.py"
		echo ">>> rather than assuming a present value is a working one."
	else
...
echo "      src/api_clients/solax_cloud_client.py's module docstring) and"
echo "      is actually valid: run 'venv/bin/python3"
echo "      scripts/solax_realtime_logger.py' and check it doesn't print"
echo "      'token invalid!' - feeds the every-5-minutes cron job in"
echo "      crontab.txt (STEP 11) that keeps data/solax_historical_data.json"
echo "      current for the evening SoC predictor and solar forecast trainer"
...
echo "      secrets.yaml above). Run 'cd ~/bin && ./forced_subs identify'"
...
echo "  [ ] healthchecks.io (home_backup): no action needed, its ping URL is"
echo "      already baked into home_backup (came back with the ~/bin git clone)"
echo "  [ ] healthchecks.io (home_automation heartbeat, STEP 11's crontab.txt"
echo "      runs it every 10m): verify secrets.yaml has a real"
echo "      healthchecks_io.ping_url (a separate check from home_backup's -"
echo "      normally restored automatically with the rest of secrets.yaml,"
echo "      same caveat as the credentials above) and that"
echo "      home_automation/config.yaml has healthchecks_io.enabled: true"
...
echo "      home_automation_hotwater home_automation_ohme"
echo "  [ ] confirm the Ohme status cache is being written (it is what keeps"
echo "      the battery daemon, hot water automation and dashboard from each"
echo "      logging in to Ohme on every poll):"
echo "        ls -l ~/home_automation/config/ohme_status.json"
echo "      A missing/stale file is not fatal - all three fall back to their"
echo "      own direct Ohme calls - but it means the poller isn't running."
echo "  [ ] confirm the pre-push test gate is installed (it is what stops a"
echo "      broken change reaching the remote this Pi pulls from):"
echo "        ls -l ~/home_automation/.git/hooks/pre-push"
echo "      Re-install with: cd ~/home_automation && bash scripts/install_git_hooks.sh"
